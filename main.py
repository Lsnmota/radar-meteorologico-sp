...
BAIRROS_SP = {
    "Sé (Centro)": (-23.5505, -46.6333), "Bela Vista": (-23.5590, -46.6534),
    "Liberdade": (-23.5583, -46.6350), "Pinheiros": (-23.5629, -46.6821),
    "Itaim Bibi": (-23.5852, -46.6819), "Moema": (-23.6003, -46.6664),
    "Vila Mariana": (-23.5875, -46.6344), "Campo Belo": (-23.6193, -46.6694),
    "Santo Amaro": (-23.6544, -46.7106), "Butantã": (-23.5709, -46.7106),
    "Lapa": (-23.5275, -46.7028), "Vila Leopoldina": (-23.5286, -46.7291),
    "Santana": (-23.5064, -46.6236), "Tucuruvi": (-23.4805, -46.6031),
    "Freguesia do Ó": (-23.4917, -46.6822), "Pirituba": (-23.4772, -46.7183),
    "Perus": (-23.4014, -46.7472), "Brasilândia": (-23.4547, -46.6825),
    "Tatuapé": (-23.5405, -46.5765), "Mooca": (-23.5605, -46.5975),
    "Penha": (-23.5303, -46.5406), "Itaquera": (-23.5361, -46.4478),
    "São Miguel Paulista": (-23.4989, -46.4444), "Guaianases": (-23.5461, -46.4106),
    "Cidade Tiradentes": (-23.5875, -46.4028), "Vila Prudente": (-23.5814, -46.5789),
    "Ipiranga": (-23.5911, -46.6079), "Cidade Ademar": (-23.6764, -46.6667),
    "Capão Redondo": (-23.6683, -46.7742), "Campo Limpo": (-23.6473, -46.7581),
    "Grajaú": (-23.7628, -46.6975), "Parelheiros": (-23.8153, -46.7517),
}
      ...
      for s in res.get("series", []):
        ...
        if vals:
          ...
          try:
            ...
            pass
  linhas = [{"cidade": nome, "populacao": populacao.get(cod)} for cod, nome in municipios.items() if populacao.get(cod) and populacao.get(cod) >= populacao_minima]
  return pd.DataFrame(linhas).sort_values("populacao", ascending=False)


def geocodificar(nome):
  if nome in COORDENADAS_ESTATICAS_SP: return COORDENADAS_ESTATICAS_SP[nome]
  ...
  for tentativa in range(1, 4):
    try:
      ...
      time.sleep(0.3)
      for res in results:
        if "são paulo" in (res.get("admin1") or "").lower(): return (res["latitude"], res["longitude"])
      if results: return (results[0]["latitude"], results[0]["longitude"])
      return None
    except Exception:
      time.sleep(1.5)
  return None
...
def buscar_dados_chuva(dict_coordenadas, data_inicio, data_fim, timezone=TIMEZONE):
  ...
  for nome, res in zip(nomes, dados):
    # Processa Probabilidade
    if "hourly" in res and "precipitation_probability" in res["hourly"]:
      df_h = pd.DataFrame({"time": pd.to_datetime(res["hourly"]["time"]), "prob": [p / 100.0 for p in res["hourly"]["precipitation_probability"]]})
    ...
    if "daily" in res and "precipitation_sum" in res["daily"]:
      df_d = pd.DataFrame({"time": pd.to_datetime(res["daily"]["time"]), "vol": res["daily"]["precipitation_sum"]})
...
def gerar_excel(df_cidades_prob, df_cidades_vol, df_bairros_prob, df_bairros_vol, caminho_saida):
  ...
  def criar_aba(ws, df_dados, eh_cidades, tipo_dado):
    ...
    # Escalas de cores diferentes para Probabilidade e Volume
    if tipo_dado == "prob":
    ...
    for c in range(2, max_col + 1): ws.column_dimensions[get_column_letter(c)].width = 10
...
def enviar_email(caminho_arquivo):
  ...
  destinatarios_raw = os.environ.get("EMAIL_DESTINATARIOS", remetente)
  destinatarios = [d.strip() for d in destinatarios_raw.split(",") if d.strip()]

  if not remetente or not senha: return print("ERRO: Credenciais ausentes.")
...
O arquivo agora contém 4 abas, contemplando a Probabilidade de Chuva (%) e o Volume Acumulado (mm) para Cidades e Bairros da Capital.
